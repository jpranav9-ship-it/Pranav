...
function buildConnectNote(prospect) {
  const firstName = prospect.name.split(' ')[0];
  return `Hi ${firstName}, came across your work in ${prospect.role}. I’m exploring how AEO and AI search are changing the way marketing teams think about visibility. Thought it would be good to connect.`;
}
...
export default function Home() {
  ...
  async function copyConnectNote(prospect) {
    try {
      await navigator.clipboard.writeText(buildConnectNote(prospect));
      setCopiedName(prospect.name);
      setTimeout(() => setCopiedName(''), 2200);
