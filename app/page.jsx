...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/usage').then((response) => response.ok ? response.json() : null).then((data) => {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true); setError(''); setProspects([]); setSearchedCompany(name); setWaitlistStatus('');
    try {
  ...
  async function saveProspect(prospect) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Could not save prospect.');
  ...
  async function handleWaitlist(e) {
    e.preventDefault(); if (joiningWaitlist) return; setJoiningWaitlist(true); setWaitlistStatus('');
    try {
      const response = await fetch('/api/waitlist', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: waitlistEmail }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Could not join the waitlist.');
      setWaitlistStatus(data.added ? 'You’re on the list. We’ll let you know when more searches are available.' : 'You’re already on the waitlist.'); setWaitlistEmail('');
    } catch (err) { setWaitlistStatus(err.message || 'Could not join the waitlist.'); }
  ...
  return (
    <main className="page">
      <nav className="nav">
        ...
        <div className="nav-actions"><span className="beta">MVP</span>{accountEmail ? <a className="nav-link" href="/dashboard">Workspace</a> : <a className="nav-link signin-link" href="/signin">Sign in</a>}</div>
      </nav>
      ...
      {!loading && !error && !limitReached && searchedCompany && <section className="results">
        <div className="results-head"><div><div className="eyebrow">PROSPECTS FOUND</div><h2>Who to reach at {searchedCompany}</h2></div><span className="count">{prospects.length} relevant prospects</span></div>
        {prospects.length > 0 ? <div className="cards">{prospects.map((p, i) => <article className="card" key={`${p.name}-${i}`}>
          <div className="card-top"><div className="avatar">{initials(p.name)}</div><div><h3>{p.name}</h3><p className="role">{p.role}</p></div><span className={`priority p${i}`}>{p.confidence || 'Relevant'}</span></div>
          <div className="detail"><span>WHY THIS PERSON</span><p>{p.why}</p></div><div className="detail"><span>AEO CONTEXT</span><p>{p.relevance}</p></div><div className="angle"><span>OUTREACH ANGLE</span><p>{p.angle}</p></div>
          <div className="card-actions"><a className="source-link" href={p.sourceUrl} target="_blank" rel="noreferrer">Open profile / evidence ↗</a><button className="save-button" type="button" onClick={() => saveProspect(p)} disabled={savingName === p.name}>{savedNames.includes(p.name) ? 'Saved ✓' : savingName === p.name ? 'Saving…' : 'Save prospect'}</button></div>
        </article>)}</div> : <div className="empty"><div className="empty-icon">?</div><p>No well-supported marketing prospects were found. Try the company name again.</p></div>}
