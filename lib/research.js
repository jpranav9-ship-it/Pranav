...
export async function researchCompany(company) {
  ...
  if (!name) throw new Error('Please enter a company name.');

  const tavilyKey = process.env.TAVILY_API_KEY;
  const geminiKey = process.env.GEMINI_API_KEY;
  if (!tavilyKey || !geminiKey) {
    throw new Error('The research service is not configured yet. Please add the API keys in Vercel and redeploy.');
  }
