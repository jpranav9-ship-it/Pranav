...
function withUserCookie(response, anonymousId, isNew) {
  if (isNew) response.cookies.set(USER_COOKIE, anonymousId, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 60 * 60 * 24 * 365, path: '/' });
  return response;
}
...
export async function POST(request) {
  ...
  try {
    ...
    if (!tavilyKey || !geminiKey) return NextResponse.json({ error: 'The research service is not configured yet. Please add the API keys in Vercel and redeploy.' }, { status: 500 });
    ...
    if (!usage.allowed) {
      const response = NextResponse.json({ error: 'You have used all 5 free searches. Upgrade to Pro for more searches.', limitReached: true, remaining: 0 }, { status: 429 });
    ...
    let parsed;
    try { parsed = JSON.parse(text); } catch { const jsonStart = text.indexOf('{'); const jsonEnd = text.lastIndexOf('}'); if (jsonStart === -1 || jsonEnd === -1) throw new Error('Research response was not valid JSON.'); parsed = JSON.parse(text.slice(jsonStart, jsonEnd + 1)); }
    ...
    if (signedInAccount?.accountId) {
      try { await convex.mutation(anyApi.history.saveSearch, { accountId: signedInAccount.accountId, company: name, prospects }); } catch (historyError) { console.error('Could not save search history:', historyError); }
    }

    const response = NextResponse.json({ company: name, prospects, sourceCount: uniqueEvidence.length, remaining: usage.remaining });
    return withUserCookie(response, anonymousId, isNewUser);
  } catch (error) {
    if (reserved && anonymousId && process.env.NEXT_PUBLIC_CONVEX_URL) { try { await getConvexClient().mutation(anyApi.usage.refundSearch, { anonymousId }); } catch (refundError) { console.error('Could not refund failed search:', refundError); } }
    console.error('Prospect research error:', error);
    return NextResponse.json({ error: error.message || 'Something went wrong while researching the company.' }, { status: 500 });
  }
}
